...
function gui_click(event)
	...
	if gui.name == "betterbots_top_btn" then
		if mod_gui.get_frame_flow(player).betterbots_left then
			...
			local main_frame = mod_gui.get_frame_flow(player).add{
				type = "frame", name = "betterbots_left", direction = "vertical"
			}
			
			main_frame.add({type = "label", name = "betterbots_left_label", caption = {"betterbots-gui.left_label"}, style = "caption_label"})
			
			main_frame.add{type = "button", name = "betterbots_reset_btn", caption = {"betterbots-gui.reset_btn"}, direction = "horizontal"}
	
			main_frame.add{ type = "label", name = "betterbots_left_label_2", caption = {"betterbots-gui.left_label_2"} }
			main_frame.add{ type = "label", name = "betterbots_left_label_3", caption = {"betterbots-gui.left_label_3"} }
		end
	elseif gui.name == "betterbots_reset_btn" then
		...
		local main_frame = player.gui.center.add{type = "frame", name = "betterbots_confirm_box", direction = "vertical"}
		
		main_frame.add{type = "label", name = "betterbots_confirm_label", caption = {"betterbots-gui.confirm_label"}, style = "caption_label"}
		main_frame.add{type = "label", name = "betterbots_confirm_notes", caption = {"betterbots-gui.confirm_notes"}}
		
		local confirm_flow = main_frame.add{type = "flow", name = "betterbots_confirm_btn_flow", direction = "horizontal"}
		confirm_flow.add{type = "button", name = "betterbots_confirm_yes_btn", caption = {"betterbots-gui.confirm_yes_btn"}}
		confirm_flow.add{type = "button", name = "betterbots_confirm_no_btn", caption = {"betterbots-gui.confirm_no_btn"}}
...
function on_tick(event)
	...
	-- 1)
	table.each(global.modified_bps, function(stack)
...
function put_roboport(event)
	...
	if techs.chargepads_tech_level > 0 and
	   stack and
	   stack.valid and
	   stack.valid_for_read and
	   stack.name == "roboport" then
		local e_x = event.position.x
		local e_y = event.position.y

		local ghosts = player.surface.find_entities_filtered({ghost_type = 'roboport', area = {{ e_x - 2, e_y - 2 },{ e_x + 2, e_y + 2}}})
		local curs_stack = player.cursor_stack

		table.each(ghosts, function(port)
...
function remove_roboport(event)
	...
	if type == 'roboport' and name:starts_with('betterbots-roboport') then
		...
		if port_data and port_data.dummy_entities then
			table.each(port_data.dummy_entities, function(ent)
...
function upgrade_roboports(force, techs)
	...
	table.each(game.surfaces, function(surface)
		...
		table.each(ghosts, function(ghost)
